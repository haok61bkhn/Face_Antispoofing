...
def get_valid_triplets_mask(labels):
    ...
    """
    indices_equal = torch.eye(labels.size(0), device=labels.device).byte()
    indices_not_equal = ~indices_equal
...
class TripletLoss(nn.Module):
    ...
    def __init__(self, margin = 0.5, squared = False):
    ...
    def forward(self, pred, target):
        ...
        pred = F.normalize(pred, p=2, dim=1)  # l2_normlize
        loss, _ = batch_all_triplet_loss(target, pred, self.margin, self.squared)
        return loss
...
if __name__ == "__main__":
    ...
    out = torch.randn(5, 7)
    sqo = pairwise_distances(out, squared=True)
    sqr = pairwise_distances(out, squared=False)
    print(sqo)
    print(sqr)